  ...
  return (
    <header className="bg-white border-b border-gray-100 h-20 sticky top-0 z-30 px-4 md:px-8 flex items-center justify-between">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-[1.3rem] font-display font-bold text-secondary truncate">{title}</h1>
      </div>
      ...
      <div className="flex items-center gap-3 md:gap-6">
        ...
        <div className="flex items-center gap-3 pl-3 md:pl-6 border-l border-gray-100">
          <div className="text-right hidden sm:block">
            <p className="text-profile-name font-bold text-secondary">{localStorage.getItem('branchName')}</p>
            <p className="text-email text-text-muted font-medium">{localStorage.getItem('staffID')}</p>
          </div>
